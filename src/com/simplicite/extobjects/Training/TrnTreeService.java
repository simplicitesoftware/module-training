...
public class TrnTreeService extends RESTServiceExternalObject {
  ...
  private JSONArray getImages(String lessonId) {
    ...
    for (String[] row : g()
        .query("select row_id, trn_pic_image from trn_picture where trn_pic_lsn_id=" + Tool.toSQL(lessonId)))
  ...
  private JSONArray getCategoriesRecursive(String parentId, JSONArray tags, String lang) {
    ...
    for (JSONObject cat : getCategories(parentId, lang)) {
      ...
      if (cat.getJSONArray("items").length() == 0)
        continue;
      cats.put(cat);
    }
    return cats;
    ...
    synchronized (tmpLesson) {
      ...
      for (int i = 0; i < rows.size(); i++) {
        ...
        try {
          ...
          if (tags.length() > 0 && !tagLsn.lessonHasTag(rows.get(i)[0], tags))
            addLessonFlag = false;
          if (addLessonFlag)
            lessons.put(lsn);
          if (i == rows.size() - 1)
