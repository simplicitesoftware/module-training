...
public class TrainingContentAdapter extends SimpleAdapter {
      ...
      for (File child : children) {
        if (child.getName().equals("content")) {
          ...
          for (File rootChild : rootChildren) {
            if (rootChild.isFile())
              AppLog.info(getClass(), "process",
                  "There should not be files at the root level of the content : " + rootChild.getName(), getGrant());
            else if (rootChild.isDirectory() && rootChild.getName().startsWith(CATEGORY_PREFIX))
              readCategoryFolder(rootChild, null);
            else if (rootChild.isDirectory() && rootChild.getName().startsWith(LESSON_PREFIX))
              readLessonFolder(rootChild, null);
            else if (rootChild.isDirectory())
              AppLog.info(getClass(), "process", "Folder does not respect naming conventions : " + rootChild.getName(),
                  getGrant());
          }
        }
      }
  ...
  private void readCategoryFolder(File categoryFolder, String parentId) {
    AppLog.info(getClass(), "readCategoryFolder", "CREATING CATEGORY FROM FOLDER " + categoryFolder.getName(),
        getGrant());
    try {
      // 1. Read category.json and create TrnCategory & TrnCategoryTranslate
      JSONObject jsonCategoryDescription = new JSONObject(
          getFileContentAsString(categoryFolder.getAbsolutePath() + "/category.json"));
      ...
      // 2. Iterate through elements and when starting with 'LSN' => call
      // createLessonFromDirectory
      for (File child : categoryFolder.listFiles()) {
        if (child.getName().startsWith(CATEGORY_PREFIX))
          readCategoryFolder(child, categoryId);
        else if (child.getName().startsWith(LESSON_PREFIX))
          readLessonFolder(child, categoryId);
      }
    } catch (IOException e) {
    ...
    for (String lang : LANGUAGES) {
      try {
        ...
        if (description != null) {
          ...
          AppLog.info(getClass(), "createCategoryTranslations", "NO DEFAULT DESCRIPTION FOR THE CATEGORY " + categoryId,
              getGrant());

      } catch (JSONException e) {
        AppLog.info(getClass(), "createCategoryTranslations", "ERROR WHILE READING category.json of " + categoryId,
            getGrant());
      } catch (ValidateException e) {
        AppLog.error(getClass(), "createCategoryTranslations",
            "ValidateException when creating a TrnCategoryTranslation", e, getGrant());
      } catch (CreateException e) {
        AppLog.error(getClass(), "createCategoryTranslations", "CreateException when creating a TrnCategoryTranslation",
            e, getGrant());
      }
    }
  }
  ...
  private void readLessonFolder(File lessonFolder, String categoryId) {
    try {
      ...
      JSONObject lessonDescription = new JSONObject(
          Files.readString(Paths.get(lessonFolder.getAbsolutePath() + "/lesson.json"), StandardCharsets.UTF_8));
      ...
      for (File file : lessonFolder.listFiles()) {
        ...
        if (isFileAContent(file.getName()))
          addContentToTranslation(language, lesson.getFieldValue("row_id"),
  ...
  private void createLessonTranslations(JSONObject lessonDescription, String lessonId) {
    for (String lang : LANGUAGES) {
      try {
        ...
        if (description != null) {
          ObjectDB translation = getGrant().getObject(lang + description.getString("title") + "_TrnLsnTranslate",
              "TrnLsnTranslate");
          ...
          AppLog.info(getClass(), "createLessonTranslations", "LESSON " + lessonId + " HAS NO " + lang + " INFORMATION",
              getGrant());
        }
      } catch (JSONException e) {
        AppLog.error(getClass(), "createCategoryTranslations", "ERROR WHILE READING lesson.json of " + lessonId, e,
            getGrant());
      } catch (ValidateException e) {
        AppLog.error(getClass(), "createCategoryTranslations", "ValidateException when creating a TrnLsnTranslation", e,
            getGrant());
      } catch (CreateException e) {
        AppLog.error(getClass(), "createCategoryTranslations", "CreateException when creating a TrnLsnTranslation", e,
            getGrant());
      } catch (Exception e) {
        AppLog.error(getClass(), "createCategoryTranslations", "GlobalException in createLessonTranslations", e,
            getGrant());
      }
    }
  }
  ...
  private void addContentToTranslation(String lang, String lessonId, String content) {
    ...
    if (rows.size() >= 1) {
      ...
      AppLog.info(getClass(), "addContentToTranslation",
          "There is no " + lang + " translation in lesson.json for lesson with ID : " + lessonId, getGrant());
    }
  }
  ...
  private void addVideoToTranslation(String lang, String lessonId, File video) {
    ...
    if (rows.size() >= 1) {
      ...
      AppLog.info(getClass(), "addVideoToTranslation",
          "There is no " + lang + " translation in lesson.json for lesson with ID : " + lessonId, getGrant());
    }
  }
  ...
  private int getOrderFromFolderName(String folderName) {
    try {
      ...
      AppLog.error(getClass(), "getOrderFromFolderName",
          "Exception while reading order from the folderName" + folderName, e, getGrant());
      return 0;
    } catch (Exception e) {
      return 0;
    }
  ...
  private String determineFileLanguage(String fileName) {
    ...
    for (String lang : LANGUAGES)
      if (fileName.contains(FOLDER_SEPARATOR + lang.toLowerCase())
          || fileName.contains(FOLDER_SEPARATOR + lang.toUpperCase()))
        language = lang;
    return language;
  }
  ...
  private void eraseParentlessLessons() {
    ...
    for (String[] row : lesson.search()) {
      ...
      try {
        ...
        AppLog.error(getClass(), "eraseRootCategories", "DeleteException while deleting parentless lessons", e,
            getGrant());
      } catch (Exception e) {
        AppLog.error(getClass(), "eraseRootCategories", "Global Exception while deleting parentless lessons", e,
            getGrant());
      }
    }
  }
