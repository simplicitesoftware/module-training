...
export default {
    ...
    methods:{
        ...
        async setSuggestion(translateRowId) {
            ...
            if(translateRes) {
                if(translateRes.trnLtrLang === this.lang) {
                    const lessonRes = await this.searchLesson(translateRes.trnLtrLsnId);
                    if(lessonRes) {
                        await this.validateAndSetSuggestion(suggestion, lessonRes, translateRes);
                    }
                } else if(translateRes.trnLtrLang === "ANY") {
                    const lessonRes = await this.searchLesson(translateRes.trnLtrLsnId);
                    if(lessonRes) {
                        const currentLangTranslate = await this.searchTranslateLsnRowId(lessonRes.row_id, this.lang);
        ...
        async validateAndSetSuggestion(suggestion, lessonRes, translateRes) {
            ...
            const catPublished = await this.isCategoryPublished(lessonRes.trnLsnCatId, true);
            if(catPublished) {
        ...
        async searchTranslateLang(rowId, lang) {
            const res = await this.translate.search({row_id: rowId, trnLtrLang: lang});
            if(res.length) {
                return res[0];
            } else {
                return null;
        ...
        async searchTranslateLsnRowId(lnsRowId, lang) {
            const res = await this.translate.search({row_id: lnsRowId, trnLtrLang: lang});
            if(res.length) {
                return res[0];
            } else {
                return null;
        ...
        async isCategoryPublished(rowId) {
            ...
            if(resCat.trnCatPublish) {
                if(resCat.trnCatId) return await this.isCategoryPublished(resCat.trnCatId);
                else return true;
            }
            return false
        },
